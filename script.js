...
function playGame() {
  ...
  let Entered_values = document.getElementById('number-guess').value
  console.log(saveGuessHistory(Entered_values));
  displayResult(Entered_values)
  displayHistory()
...
function displayResult(Entered_values) {

  if (correct_number == Entered_values) {
    showYouWon()
  }
  else if (Entered_values > correct_number) {
    showNumberAbove()
  }
  else if (Entered_values < correct_number) {
    showNumberBelow()
...
function displayHistory() {
  let index=guessed.length-1; // TODO
  let list = "<ul class='list-group'>";
  // *CODE GOES BELOW HERE *
  while (index >= 0) {
    list += "<li class='list-group-item'>" + "You Guesses "
      + guessed[index] + "</li>"
    index--;
  }
  list += '</ul>'
